        ...
        private void MtDadosCR_LostFocusAfter(object sboObject, SBOItemEventArg pVal)
        {
            if ((pVal.CharPressed == 0 || pVal.CharPressed == 32) && (pVal.ColUID == "Col_18" || pVal.ColUID == "Col_19" || pVal.ColUID == "Col_28" || pVal.ColUID == "Col_31" || pVal.ColUID == "Col_32"))
            {
                try
                {
                    ...
                    if (double.Parse(((EditText)MtDadosCR.Columns.Item("Col_28").Cells.Item(pVal.Row).Specific).Value)
                        >
                        double.Parse(((EditText)MtDadosCR.Columns.Item("Col_26").Cells.Item(pVal.Row).Specific).Value))
                    {
                        ...
                        ((EditText)MtDadosCR.Columns.Item("Col_28").Cells.Item(pVal.Row).Specific).Value = "0";
                    }
                    else
                    {
                        ...
                        if (pVal.ColUID == "Col_32" && (DescPOri != Math.Round(double.Parse(DtDadosCR.GetValue("DescP", pVal.Row - 1).ToString()), 2)))
                        {
                            ...
                            DtDadosCR.SetValue("Desconto", pVal.Row - 1, (double.Parse(((EditText)MtDadosCR.Columns.Item("Col_32").Cells.Item(pVal.Row).Specific).Value.Replace(".", ",")) / 100)
                                *
                                double.Parse(((EditText)MtDadosCR.Columns.Item("Col_28").Cells.Item(pVal.Row).Specific).Value.Replace(".", ",")));

                            DtDadosCR.SetValue("ValorAtual", pVal.Row - 1, (double.Parse(((EditText)MtDadosCR.Columns.Item("Col_28").Cells.Item(pVal.Row).Specific).Value.Replace(".", ",")) - (double.Parse(((EditText)MtDadosCR.Columns.Item("Col_32").Cells.Item(pVal.Row).Specific).Value.Replace(".", ",")) / 100)
                                *
                                double.Parse(((EditText)MtDadosCR.Columns.Item("Col_28").Cells.Item(pVal.Row).Specific).Value.Replace(".", ","))));
                        ...
                        {
                            if (double.Parse(((EditText)MtDadosCR.Columns.Item("Col_18").Cells.Item(pVal.Row).Specific).Value.Replace(".", ",")) > double.Parse(((EditText)MtDadosCR.Columns.Item("Col_28").Cells.Item(pVal.Row).Specific).Value.Replace(".", ","))
                               ||
                            ...
                            DtDadosCR.SetValue("ValorAtual", pVal.Row - 1, (double.Parse(((EditText)MtDadosCR.Columns.Item("Col_28").Cells.Item(pVal.Row).Specific).Value.Replace(".", ","))
                                -
                        ...
                        else if (pVal.ColUID == "Col_28" && (DescOri != Math.Round(double.Parse(DtDadosCR.GetValue("ValorAReceber", pVal.Row - 1).ToString()), 2)))
                        {
                            ...
                            DtDadosCR.SetValue("ValorAtual", pVal.Row - 1, Math.Round(double.Parse(DtDadosCR.GetValue("ValorAReceber", pVal.Row - 1).ToString()), 2));
                        }
                        //-------------------------------------------------------------------------------------------------------------------------------------- Valor Outros
                        else if (pVal.ColUID == "Col_19" && (OutrosOri != Math.Round(double.Parse(DtDadosCR.GetValue("Outros", pVal.Row - 1).ToString()), 2)))
                        {
                            ...
                            {
                                if (Math.Round(double.Parse(DtDadosCR.GetValue("Outros", pVal.Row - 1).ToString()), 2) < (Math.Round(double.Parse(DtDadosCR.GetValue("ValorAReceber", pVal.Row - 1).ToString()), 2) * -1))
                                {
                                ...
                                {
                                    DtDadosCR.SetValue("ValorAtual", pVal.Row - 1, Math.Round(double.Parse(DtDadosCR.GetValue("ValorAReceber", pVal.Row - 1).ToString()), 2) + Math.Round(double.Parse(DtDadosCR.GetValue("Outros", pVal.Row - 1).ToString()), 2));
        ...
        private void BtEfetivar_PressedAfter(object sboObject, SBOItemEventArg pVal)
        {
            try
            {
                ...
                var filteredRowsNf = from row in Enumerable.Range(0, DtDadosCR.Rows.Count)
                                     ...
                                     select new
                                     {
                                         OF = DtDadosCR.GetValue("OF", row),
                                         ValorAReceber = DtDadosCR.GetValue("ValorAReceber", row),
                                         DataPagto = DtPag.Value,
                                         TipoDoc = DtDadosCR.GetValue("TIPO", row),
                                         CodLinDoc = DtDadosCR.GetValue("LINUN", row),
                                         ValorLiq = DtDadosCR.GetValue("ValorLiquido", row),
                                         Cliente = DtDadosCR.GetValue("Cliente", row),
                                         CodDocR = DtDadosCR.GetValue("CodDocR", row),
                                         ValorDoc = DtDadosCR.GetValue("ValorTitulo", row),
                                         Fatura = DtDadosCR.GetValue("Fatura", row),
                                         Obra = DtDadosCR.GetValue("Obra", row),
                                         Site = DtDadosCR.GetValue("Local", row),
                ...
                foreach (var row in filteredRowsNf)
                {
                    #region DIAPI
                    ...
                    //    //if (vPay.GetByKey(2340))
                    //    //{
                    //    //    vPay.SaveXML(@"C:\Temp\CRTeste.XML");
                    //    //}

                    //    vPay.CardCode = row.Cliente.ToString();// "103000";

                    //    //vPay.CashAccount = "11001004000002";

                    //    //vPay.CashSum = 100;

                    //    vPay.BPLID = 1;

                    //    //vPay.CheckAccount = "11001004000002";

                    //    //vPay.BankAccount  = "11001004000002";

                    //    //vPay.PayToBankAccountNo = "11001004000002";

                    //    vPay.TransferAccount = "11001004000002";

                    //    //vPay.ContactPersonCode = 0;

                    //    //vPay.DocCurrency = "GBP";
                    ...
                    //    //var tempA = row.ValorAReceber.ToString();
                    //    //var tempB = row.ValorLiq.ToString();

                    //    vPay.TransferSum = double.Parse(row.ValorAReceber.ToString());

                    //    //vPay.AccountPayments.AccountCode = "11001004000002";                       

                    //    //vPay.AccountPayments.SumPaid = double.Parse(row.ValorLiq.ToString());
                    ...
                    //    vPay.Invoices.SumApplied  = double.Parse(row.ValorDoc.ToString());
