...
BEGIN
	BEGIN TRY
		...
		SELECT 
			...
			ISNULL(IMP.CSLLWTAmnt, 0)* (inv6.InstPrcnt/100)  AS valorcsll,
			ISNULL(IMP.INSSWTAmnt, 0) * (inv6.InstPrcnt/100) AS valorinss,    
			ISNULL(IMP.IRRFWTAmnt, 0)* (inv6.InstPrcnt/100)  AS valorirrf,
			ISNULL(IMP.ISSWTAmnt, 0) * (inv6.InstPrcnt/100) AS valoriss,
